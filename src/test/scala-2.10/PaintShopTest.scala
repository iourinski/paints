...
class PaintShopTest extends TestKit(ActorSystem("PaintOrderSystem")) with ImplicitSender
  ...
  "A worker actor" must {
    ...
    s"""gives identical answers whether we use arrays or hashmaps for order tracking""" in {
      assert(
        worker.underlyingActor.processCase(goodOrder.customers, goodOrder.numPaints).deep ==
          worker.underlyingActor.processCaseWithHash(goodOrder.customers, goodOrder.numPaints).deep
      )
    }
  }
}
